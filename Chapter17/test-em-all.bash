...
function testCompositeCreated() {
    ...
    if ! assertCurl 200 "curl $AUTH -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS -s"
    then
        echo -n "FAIL"
        return 1
    fi
...
function recreateComposite() {
    ...
    assertCurl 200 "curl $AUTH -X DELETE -k https://minikube.me/product-composite/${productId} -s"
    curl -X POST -k https://minikube.me/product-composite -H "Content-Type: application/json" -H "Authorization: Bearer $ACCESS_TOKEN" --data "$composite"
}
...
function testCircuitBreaker() {
    ...
    then
        ...
        kubectl -n $ns run --restart=Never alpine-client --image=alpine --command -- sleep 6000
    ...
    for ((n=0; n<3; n++))
    do
        assertCurl 500 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
    ...
    assertCurl 200 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS?delay=3 $AUTH -s"
    ...
    assertCurl 200 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS $AUTH -s"
    assertEqual "Fallback product2" "$(echo "$RESPONSE" | jq -r .name)"

    # Verify that a 404 (Not Found) error is returned for a non existing productId ($PROD_ID_NOT_FOUND) from the fallback method.
    assertCurl 404 "curl -k https://minikube.me/product-composite/$PROD_ID_NOT_FOUND $AUTH -s"
    ...
    echo "Will sleep for 10 sec waiting for the CB to go Half Open...$HOST"
    sleep 10

    # Verify that the circuit breaker is in half open state
    #assertEqual "HALF_OPEN" "$($EXEC wget https://minikube.me/product-composite/actuator/health -qO - | jq -r .components.circuitBreakers.details.product.details.state)"
    ...
    assertEqual "CLOSED" "$($EXEC wget https://minikube.me/product-composite/actuator/health -qO - | jq -r .components.circuitBreakers.details.product.details.state)"

    # Verify that the expected state transitions happened in the circuit breaker
    assertEqual "CLOSED_TO_OPEN"      "$($EXEC wget https://minikube.me/product-composite/actuator/circuitbreakerevents/product/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$($EXEC wget https://minikube.me/product-composite/actuator/circuitbreakerevents/product/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$($EXEC wget https://minikube.me/product-composite/actuator/circuitbreakerevents/product/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-1].stateTransition)"
...
waitForService curl -k https://minikube.me/actuator/health
...
assertCurl 200 "curl -k https://minikube.me/product-composite/$PROD_ID_NO_RECS $AUTH -s"
...
assertCurl 200 "curl -k https://minikube.me/product-composite/$PROD_ID_NO_REVS $AUTH -s"
assertEqual $PROD_ID_NO_REVS $(echo $RESPONSE | jq .productId)
assertEqual 3 $(echo $RESPONSE | jq ".recommendations | length")
assertEqual 0 $(echo $RESPONSE | jq ".reviews | length")

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl -k https://minikube.me/product-composite/-1 $AUTH -s"
assertEqual "\"Invalid productId: -1\"" "$(echo $RESPONSE | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl -k https://minikube.me/product-composite/invalidProductId $AUTH -s"
assertEqual "\"Type mismatch.\"" "$(echo $RESPONSE | jq .message)"

# Verify that a request without access token fails on 401, Unauthorized
assertCurl 401 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS -s"

# Verify that the reader - client with only read scope can call the read API but not delete API.
READER_ACCESS_TOKEN=$(curl -k https://reader:secret@minikube.me/oauth/token -d grant_type=password -d username=magnus -d password=password -s | jq .access_token -r)
READER_AUTH="-H \"Authorization: Bearer $READER_ACCESS_TOKEN\""

assertCurl 200 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS $READER_AUTH -s"
assertCurl 403 "curl -k https://minikube.me/product-composite/$PROD_ID_REVS_RECS $READER_AUTH -X DELETE -s"
